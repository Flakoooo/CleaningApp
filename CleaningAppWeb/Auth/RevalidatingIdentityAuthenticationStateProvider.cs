...
namespace CleaningAppWeb.Auth
{
    ...
    ) : RevalidatingServerAuthenticationStateProvider(loggerFactory) where TUser : class
    {
        ...
        private async Task<bool> ValidateSecurityStampAsync(UserManager<TUser> userManager, ClaimsPrincipal principal)
        {
            ...
            var securityStamp = principal.FindFirstValue(_options.ClaimsIdentity.SecurityStampClaimType);
            return securityStamp == await userManager.GetSecurityStampAsync(user);
        }
    }
}
